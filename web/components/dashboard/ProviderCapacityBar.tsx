...
export default function ProviderCapacityBar({ providers }: ProviderCapacityBarProps) {
  ...
  return (
    <div className="cf-panel rounded-[28px] p-6">
      <h3 className="mb-4 text-lg font-semibold text-[var(--cf-text-0)]">
        Storage by Provider
      </h3>
      ...
      {providersWithQuota.length > 0 && (
        <div className="space-y-4 mb-6">
          {providersWithQuota.map((provider) => {
            ...
            return (
              <div key={`${provider.providerId}:${provider.accountKey || provider.accountEmail || provider.displayName || 'default'}`} className="space-y-2 rounded-[20px] border border-[var(--cf-border)] bg-[var(--cf-panel-soft)] p-3">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <div className="flex h-6 w-6 items-center justify-center overflow-hidden rounded bg-[rgba(255,255,255,0.08)]">
                      <svg className="h-4 w-4 text-[var(--cf-text-1)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div
                  className="h-2 overflow-hidden rounded-full bg-[var(--cf-bg3)]"
                  role="progressbar"
                ...
                <div className="flex justify-between text-xs text-[var(--cf-text-2)]">
      ...
      {providersWithoutQuota.length > 0 && (
        <div>
          <h4 className="mb-3 text-sm font-medium text-[var(--cf-text-2)]">

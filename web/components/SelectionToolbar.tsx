...
export default function SelectionToolbar({
  ...
  return (
    <div 
      ...
      className="fixed bottom-8 left-1/2 z-[100] flex -translate-x-1/2 items-center gap-6 rounded-2xl border border-[var(--cf-border-2)] bg-[var(--cf-menu-bg)] px-6 py-3 text-[var(--cf-text-0)] shadow-[var(--cf-shadow-strong)] animate-in fade-in slide-in-from-bottom-4 duration-300"
    >
      <div className="flex items-center gap-3 border-r border-[var(--cf-border)] pr-6">
        <button 
      ...
      <div className="flex items-center gap-1">
        ...
        <div className="mx-2 h-6 w-px bg-[var(--cf-border)]" />

        <button
          onClick={() => onDelete(selectedFiles)}
          className="flex items-center gap-2 rounded-lg px-3 py-1.5 text-[var(--cf-red)] transition-colors hover:bg-[rgba(255,92,92,0.12)]"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function ToolbarButton({ label, onClick, icon }: { label: string, onClick: () => void, icon: React.ReactNode }) {
  return (
    <button
      onClick={onClick}
      className="group flex flex-col items-center gap-0.5 rounded-lg px-3 py-1 transition-colors hover:bg-[var(--cf-hover-bg-strong)]"
    >

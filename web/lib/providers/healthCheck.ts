...
export type HealthStatus = 'healthy' | 'needs_reauth' | 'degraded' | 'unknown'
...
const PROBE_CONFIGS: Partial<Record<ProviderId, ProbeConfig>> = {
  ...
  pcloud: { url: 'https://api.pcloud.com/userinfo', useQueryParam: true },
  yandex: { url: 'https://login.yandex.ru/info', bearerPrefix: 'OAuth' },
}

const PROBE_TIMEOUT_MS = 2000

export async function probeProvider(providerId: string, accessToken: string): Promise<HealthProbeResult> {
  ...
  try {
    ...
    if (config.useQueryParam) {
      ...
      headers.Authorization = `${config.bearerPrefix ?? 'Bearer'} ${accessToken}`
    }
...
function buildResult(httpStatus: number, checkedAt: string, latencyMs: number): HealthProbeResult {
  ...
  return {
    ...
    checkedAt,
    httpStatus,
    message: httpStatus === 429 ? 'Provider is rate-limiting this account' : `Unexpected HTTP ${httpStatus} from provider`,
    latencyMs,
  }
}

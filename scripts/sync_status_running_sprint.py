...
def render_block(sprint: int, rows: list[dict]) -> str:
    lines = [
        ...
        "   `./scripts/finish_task.sh <task_key> --test \"<targeted test>\" --commit \"<message>\"`",
        "2. `finish_task.sh` handles commit/push/release in one flow.",
        "3. Do not update shared dashboard/metrics files from worker agents.",
        "4. Codex side (after agent push + lock release): run task completion sync and dashboard/metrics refresh.",
        "5. Codex finalize command:",
